...
from analytics_core import (
    ...
    bootstrap_analytics_tables, get_db, safe_div, normalize_exit_reason
)
...
def _classify_loss(t: dict) -> tuple[str, str]:
    ...
    exit_r = normalize_exit_reason(t.get("exit_reason"))
    ...
    if any(token in mom for token in ("BEARISH", "NEGATIVE", "WEAK", "DOWN", "DECLINING")):
    ...
    if hold > 20 and exit_r in ("SL_HIT", "INITIAL_SL"):
        primary = "LATE_ENTRY"
        return primary, secondary
    ...
    if mae > 5: signals += 1
    if hold < 15 and exit_r in ("SL_HIT", "INITIAL_SL"): signals += 1
    if signals >= 2:
...
def _cli():
    import argparse
    p = argparse.ArgumentParser(description="Loss Autopsy Engine")
    ...
    a = p.parse_args()
    r = run_loss_autopsy(strategy=a.strategy, instrument=a.instrument,
    ...
    s = r["summary"]
    print("=" * 60)
    print(f"  LOSS AUTOPSY  ·  {s['total_losses']} losses / {s['total_trades']} trades "
          f"·  {s['classified_pct']:.0f}% classified")
    print("=" * 60)
    print("  Failure causes (most damaging first):")
    for c in r["cause_ranking"]:
        print(f"   {c['count']:>3}x  Rs {c['total_loss']:>9,.0f}  {c['label']}  ({c['type']})")
    print(f"  Behavioral: Rs {s['behavioral_loss']:,.0f} ({s['behavioral_count']}) · "
          f"System: Rs {s['system_loss']:,.0f} ({s['system_count']})")
