...
# Prefer the local PIT_coco copy of db_logger so this analytics suite is fully
# self-contained and runnable standalone. Fall back to ~/openalgo (where the
# live strategies keep their copy) if not found locally.
...
def _enrich(r: dict) -> dict:
    ...
    r["exit_reason_code"] = normalize_exit_reason(r.get("exit_reason"))
...
def normalize_exit_reason(reason: str) -> str:
    """Return a stable analytics code for strategy-specific exit labels."""
    key = (reason or "").strip().upper().replace("-", "_").replace(" ", "_")
    aliases = {
        "SL": "SL_HIT",
        "STOP": "SL_HIT",
        "STOP_LOSS": "SL_HIT",
        ...
        "HIGH_WATER": "HIGH_WATER_EXIT",
        "HIGH_WATER_EXIT": "HIGH_WATER_EXIT",
        "HARD_EXIT": "HARD_EXIT",
        "NO_MOMENTUM_EXIT": "NO_MOMENTUM_EXIT",
