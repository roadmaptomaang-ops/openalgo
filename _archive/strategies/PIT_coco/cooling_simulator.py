...
def _cli():
    import argparse
    p = argparse.ArgumentParser(description="Cooling Period Simulator")
    ...
    a = p.parse_args()
    r = run_cooling_simulator(strategy=a.strategy, instrument=a.instrument,
    ...
    act = r["actual_stats"]
    print("=" * 70)
    print(f"  COOLING SIMULATOR  ·  actual net Rs {act['net_pnl']:,.0f} on {act['n']} trades")
    print("=" * 70)
    print(f"  {'Trigger':<30}{'CD':>5}{'Removed':>9}{'Sim P&L':>12}{'Delta':>12}  Verdict")
    for trig, x in r["trigger_summary"].items():
        d = x["delta"]["pnl"]
        print(f"  {x['trigger_label']:<30}{x['cooldown_min']:>4}m{x['trades_removed']:>9}"
              f"{x['simulated']['net_pnl']:>12,.0f}{d:>12,.0f}  {x['verdict']}")
    if r["best_recommendation"]:
        b = r["best_recommendation"]
        print(f"  BEST: {b['trigger_label']} · {b['cooldown_min']}min  "
              f"(+Rs {b['delta']['pnl']:,.0f})")
