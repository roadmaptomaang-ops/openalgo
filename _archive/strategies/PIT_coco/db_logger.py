...
class DBLogger:
    ...
    def insert_trade(self, row: dict) -> Optional[int]:
        """Insert one completed trade row. Returns lastrowid or None on error."""
        cols = [
    ...
    def query(self, sql: str, params: tuple = ()) -> list:
        """Read-only SELECT helper. Returns list of dict rows."""
        with self._lock:
            try:
                conn = self._connect()
    ...
    def execute(self, sql: str, params: tuple = ()) -> int:
        """
        Write helper for INSERT/UPDATE/DELETE.

        IMPORTANT: query() above does NOT commit, so any write routed through it
        was silently rolled back when the connection closed. All analytics
        modules (edge_decay, loss_autopsy, whatif, cooling_simulator) persist
        their results through this method instead. Returns lastrowid (-1 on error).
        """
        with self._lock:
            try:
                conn = self._connect()
