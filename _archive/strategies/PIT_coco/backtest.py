...
def backtest(df: pd.DataFrame, trail_atr: float = 2.0,
    ...
    if df is None or df.empty:
        return {
            "trades": [], "equity": equity, "capital": capital,
            "final_equity": capital, "total_return": 0, "n": 0,
    ...
    df_bt = df.copy()
    required = {"Close", "ATR", "cross"}
    missing = required - set(df_bt.columns)
    if missing:
        raise ValueError(f"Backtest dataframe missing columns: {sorted(missing)}")
        ...
        if position != 0:
            ...
            if position == 1:
                ...
                if price >= trail_stop or cross == 1:
                    ...
                    cash  += entry_price * shares + pnl  # release margin plus P&L
