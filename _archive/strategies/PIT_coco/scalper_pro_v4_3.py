...
MIN_HOLD_SECONDS      = 60
CANDLE_CONFIRM_COUNT  = 3   # 3 × 10s polls = 30s of consistent signal before entry
LATE_ENTRY_CUTOFF     = "14:30"  # no new entries in final hour
...
INSTRUMENTS = {
    "NIFTY": {
        ...
        "trail_step":      2,
        "adx_threshold":   25,
        "adx_max":         50,   # skip if ADX above this (exhausted move)
        "orb_candles":     3,
        "orb_buffer":      20,
        "high_water_drop": 3,    # pts drop from peak to trigger high-water exit
    },
    "SENSEX": {
        "index_symbol":    "SENSEX",
        ...
        "orb_candles":     3,
        "orb_buffer":      60,
        "high_water_drop": 7,    # wider drop for SENSEX (larger premium swings)
    }
}

# ============================================================
# LOGGING SETUP  — uses shared trade_logger module
#   logs/YYYY/MM-Month/DD/scalper_{trades,activity}.log + .csv
#   logs/scalper_master.csv
# ============================================================

from trade_logger import TradeLogger

_TL = TradeLogger("scalper")
...
def tlog(text):
    """Write to trade log (no name prefix) + terminal."""
    _TL.raw_trade(text)

def alog(text):
    """Write to activity log (no name prefix) + terminal."""
    # raw activity line without extra timestamp
    print(text)
...
def run_instrument(name, cfg):
    ...
    while True:
        ...
        if last_target_time:
            elapsed = (get_ist_now() - last_target_time).seconds
        ...
        if adx > cfg["adx_max"]:
            log(name, f"High ADX ({adx:.1f}) — exhausted move. Skipping.")
            signal_history.clear()
            time.sleep(20)
            continue
