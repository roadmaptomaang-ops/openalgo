"""
Scalper V5.2 — "Rider" Edition + Re-Entry Guard
============================================================
...
STRATEGY_TAG = "Scalper V5.2 Rider"   # tag sent to OpenAlgo on each order
...
# ── ExcludeZeroMFE fast bailout (NEW in V5.2) ─────────────
# SAGE's #1 what-if fix (+Rs3,729 ADOPT): trades whose MFE never goes positive are
# near-certain losers. Instead of riding to the full hard SL, bail the moment it's
# clear the trade never worked — after ZERO_MFE_SECS, if peak profit is still at/below
# ZERO_MFE_PTS, exit now. Cuts the "straight to SL, MFE≈0" bad-entry bleed.
ZERO_MFE_ENABLED      = True
ZERO_MFE_SECS         = 90        # give it this long to show ANY momentum
ZERO_MFE_PTS          = 1.0       # peak must clear this by then, else bail

# ── Per-day trade cap vs DB (NEW in V5.2) ─────────────────
# MAX_TRADES resets per process, so reruns re-arm a fresh 5-trade budget and re-bleed.
# V5.2 seeds today's count from trading.db at startup so a rerun CONTINUES the day's
# count instead of restarting it — the "don't-rerun" discipline enforced in code.
PERDAY_CAP_FROM_DB    = True
...
# One row per instrument per day, appended to logs/scalper_v52_funnel.csv.
FUNNEL_COLUMNS = [
...
def write_funnel(name, funnel):
    import csv as _csv
    path = _TL.root / "scalper_v52_funnel.csv"
...
def log_day_start(name):
    ...
    tlog("")
    tlog(sep)
    tlog(f"# {name} V5.2 RIDER Started — {get_ist_now().strftime('%Y-%m-%d')}")
    ...
    tlog(sep)
    tlog("")
    alog(f"# {name} V5.2 Activity Log — {get_ist_now().strftime('%Y-%m-%d')}")
    alog("")
...
def _today_trade_count(instrument):
    """Count today's scalper_v52 trades for this instrument in trading.db.
    Used to seed the per-day cap so reruns continue the day's budget."""
    try:
        import sqlite3, os
        path = os.path.expanduser("~/openalgo/trading.db")
        c = sqlite3.connect(path)
        today = get_ist_now().strftime("%Y-%m-%d")
        n = c.execute(
            "SELECT COUNT(*) FROM trades WHERE date=? AND strategy='scalper_v52' AND instrument=?",
            (today, instrument),
        ).fetchone()[0]
        c.close()
        return int(n)
    except Exception as e:
        log(instrument, f"Per-day cap seed failed (non-fatal): {e}")
        return 0
...
def log_trade(
    ...
    lines = [
        "",
        sep,
        f"TRADE #{trade_num} | {name} V5.2 | {entry_time.strftime('%Y-%m-%d')} | {entry_time.strftime('%H:%M:%S')}",
        sep,
        "",
...
def run_instrument(name, cfg):
    log(name, f"V5.2 RIDER Started | Qty:{cfg['quantity']} "
    ...
    total_pnl        = 0
    trade_count      = 0
    # V5.2 per-day cap: seed today's count from the DB so a RERUN continues the day's
    # trade budget instead of resetting it (kills the rerun-rebleed). MAX_TRADES then
    # caps the DAY, not the process.
    if PERDAY_CAP_FROM_DB:
        seeded = _today_trade_count(name)
        if seeded:
            trade_count = seeded
            log(name, f"Per-day cap: {seeded} {name} trades already in DB today — "
                      f"continuing toward MAX_TRADES={MAX_TRADES} (not restarting).")
    last_sl_time     = None
    last_exit_time   = None
    ...
    while True:
        ...
        while position_open:
            ...
            # 3. ExcludeZeroMFE (V5.2) — never went green; don't ride a dead trade to
            #    the full SL. SAGE's top what-if fix (+Rs3,729).
            if (ZERO_MFE_ENABLED and held_secs >= ZERO_MFE_SECS
                    and high_water_mark <= ZERO_MFE_PTS):
                place_order(option_symbol, cfg["option_exchange"], "SELL", cfg["quantity"])
                total_pnl += pnl_rs
                last_exit_time = get_ist_now()
                position_open  = False
                tradelog(name, f"ZERO-MFE BAILOUT ({held_secs:.0f}s, peak {high_water_mark:.1f}pts) "
                               f"| Rs {pnl_rs:.0f} | Total: Rs {total_pnl:.0f}")
                finalize("Zero-MFE Bailout", pnl_pts, pnl_rs, curr)
                break
...
def main():
    tlog("=" * 60)
    tlog("Scalper V5.2 — RIDER Edition + Re-Entry Guard — NIFTY + SENSEX")
    ...
    tlog("")
    alog("=" * 60)
    alog("V5.2 Activity Log Started")
    alog("=" * 60)
    alog("")
    ...
    tlog("=" * 60)
    tlog("V5.2 Rider complete for the day.")
