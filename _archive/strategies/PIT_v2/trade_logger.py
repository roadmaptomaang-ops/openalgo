...
class TradeLogger:
    ...
    def diagnose(
        ...
        """
        exit_code = TradeLogger._exit_code(exit_reason)
        # ── Win diagnoses ────────────────────────────────────
        if pnl_pts > 0:
            if exit_code == "TARGET_HIT":
                return ("CLEAN WIN", "Target hit with full momentum. Strategy worked as intended.")
            if exit_code == "HIGH_WATER_EXIT" and mfe_pts > 0:
        ...
        if exit_code in ("SL_HIT", "INITIAL_SL"):
            if mfe_pts <= 1.0:
