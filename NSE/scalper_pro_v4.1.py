"""
Production Momentum Scalper V4.1
==================================
...
LOG_DIR = Path(os.path.expanduser("~/openalgo/scalper_pro_trade_logs"))
...
def tlog(text):
    print(text)
    try:
        with open(get_log_file(), "a", encoding="utf-8") as fh:
            fh.write(text + "\n")
    except Exception:
        pass
...
def analyze_option_chain(chain_data, atm_strike, strike_interval, num_strikes=10):
    ...
    for row in chain:
        if abs(row["strike"] - atm) <= num_strikes * strike_interval:
            ce_oi_total += row.get("ce", {}).get("oi", 0) or 0
            pe_oi_total += row.get("pe", {}).get("oi", 0) or 0

    oi_bias = "CE" if ce_oi_total > pe_oi_total else "PE"
...
def run_instrument(name, cfg):
    ...
    while True:
        ...
        if trade_count >= MAX_TRADES:
            log(name, f"Max {MAX_TRADES} trades done. Stopping.")
            log_day_summary(name, trade_count, total_pnl, trades_detail)
            break
        ...
        if last_target_time:
            elapsed = (datetime.now() - last_target_time).seconds
        ...
        if (last_chain_fetch is None or
                (now - last_chain_fetch).seconds >= 900):
        ...
        if max_pain_bias and max_pain_bias != option_type:
            log(name, f"Max pain ({max_pain_bias}) conflicts signal ({option_type}). Skipping.")
            signal_history.clear()
            time.sleep(SIGNAL_INTERVAL)
            continue
        ...
        if (last_direction == option_type and last_sl_time and
                (datetime.now() - last_sl_time).seconds < 600):
        ...
        signal_history.clear()
        log(name, f"Trade #{trade_count} | {option_type} ITM | Entry: Rs {entry} | {option_symbol}")
        ...
        while position_open:
            ...
            if now_str >= EXIT_TIME:
                ...
                pnl_rs         = (curr - entry) * cfg["quantity"]
                total_pnl     += pnl_rs
            ...
            # High water exit
            hw_drop = 3 if high_water_mark < 15 else HIGH_WATER_DROP
            if (high_water_mark >= cfg["be_trigger"] and
                    pnl_pts <= high_water_mark - hw_drop):
                place_order(option_symbol, cfg["option_exchange"], "SELL", cfg["quantity"])
                total_pnl        += pnl_rs
                last_target_time  = datetime.now()
                ...
                log(name, f"HIGH WATER EXIT! Peak:{high_water_mark:.1f} Now:{pnl_pts:.1f} | Rs {pnl_rs:.0f} | Total: Rs {total_pnl:.0f}")
                log_trade(
                    name, trade_count, option_type, option_symbol,
                    entry, curr, exit_reason_str,
                    entry_time_dt, get_ist_now(),
            ...
            if pnl_pts >= cfg["be_trigger"] and not sl_to_cost:
                ...
                log(name, f"SL moved to break-even at +{pnl_pts:.1f}pts.")
            ...
            if pnl_pts >= cfg["trail_trigger"]:
                ...
                if new_locked > locked_profit:
                    ...
                    log(name, f"Trail #{len(trail_log)}: P&L {pnl_pts:.1f}pts | SL {trail_log[-1]['old_sl']:.1f} → {dynamic_sl:.1f}pts")
...
def main():
    tlog("=" * 60)
    tlog("Momentum Scalper V4.1 — NIFTY + SENSEX")
    tlog("OI + Max Pain + ORB + EMA + VWAP + ADX + MFE/MAE + Logging")
    tlog("=" * 60)
    tlog(f"Trade logs: {LOG_DIR}")
    tlog("")
