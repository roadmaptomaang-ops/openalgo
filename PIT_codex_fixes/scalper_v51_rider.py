"""
Scalper V5.1 — "Rider" Edition + Re-Entry Guard
============================================================
Identical to V5 Rider in every way EXCEPT it adds a re-entry guard — the one
gap that made V5 breakeven instead of green. Runs as its own strategy
("scalper_v51") so you can A/B it directly against plain V5.

WHY V5.1 EXISTS  (the re-entry guard)
-------------------------------------
V5's edge is real on trend days, but it loses money re-entering the SAME
direction 2-4 minutes after a winner — into a bounce/exhausted tail, at an
inflated premium. Documented cases:
  • 29-Jun SENSEX #2 (-Rs475) and #5 (-Rs380): both re-entered right after a
    winner, into a pause, MFE ~0, straight to SL.
  • 01-Jul Session-2 NIFTY #2 (-Rs500) and #5 (-Rs543): same chase pattern.
These are NOT stopped by stricter entry filters (the chasers often had
STRONGER ADX than the winners — ADX lags). The fix is behavioural.

THE GUARD (see REENTRY_GUARD below):
  After a trade in a given direction closes, a NEW trade in the SAME direction
  is only allowed once the underlying SPOT has made a FRESH EXTREME beyond the
  previous same-direction ENTRY spot, by at least `orb_buffer` points:
     • CE re-entry needs   spot > last_CE_entry_spot + orb_buffer  (fresh high)
     • PE re-entry needs   spot < last_PE_entry_spot - orb_buffer  (fresh low)
  Continuation trades (the trend keeps running) pass. Chop re-entries at the
  same/pulled-back level are blocked. The OPPOSITE direction is never blocked
  (a genuine reversal should always be tradable).

EVERYTHING ELSE IS V5:
  • ENTRY  : ORB breakout + EMA9>EMA21 + price vs VWAP + ADX floor (+ nan guard).
             NO ADX ceiling — strong trends are taken.
  • RIDE   : no fixed target. Hold while the underlying trend is intact.
  • EXIT   : initial hard SL / wide trailing SL / momentum-flip / hard time exit.
  • GUARDS : stale-quote protective exit + shared circuit breaker (both kept).
...
Run:
    ...
    uv run python3 scalper_v51_rider.py

CAUTION: V5.1 trades the SAME index options as V5 and V4.3. Do NOT run more
than one of them LIVE at once (double exposure). For an A/B, run in
...
STRATEGY_TAG = "Scalper V5.1 Rider"   # tag sent to OpenAlgo on each order
...
MONITOR_INTERVAL_HOT  = 1         # ...but once TRAILING (deep in profit), poll fast.
                                  # 2026-07-08: SENSEX trade locked +111.3pts, filled
                                  # +55.1 — Rs1,123 gapped through the trail in one 5s
                                  # polling window. The trail is a trigger, not a fill;
                                  # a 1s watch shrinks the worst-case gap ~5x.
...
# ── Re-entry guard (NEW in V5.1) ──────────────────────────
# Block a same-direction re-entry until spot makes a fresh extreme beyond the
# previous same-direction ENTRY spot by at least `orb_buffer` points. The
# opposite direction is never blocked. Set False to behave exactly like V5.
REENTRY_GUARD         = True

# ── Chase gate (NEW, 2026-07-08) ──────────────────────────
# Backtested on the full trade history (trading.db, all engines): the 2nd+
# same-direction entry taken right after the PRIOR same-direction entry WON
# is the worst-performing trade type in the entire book (2nd-trade-after-win
# avg Rs-157 vs 2nd-trade-after-loss avg Rs-17 — chasing a winner is far worse
# than re-entering after a stop). Backtest on scalper_v51 specifically:
# actual net -Rs3,398 -> with gate -Rs938 (+Rs2,460). Validated -> ON by default
# here. (Same gate on trend_rider_v2/MINT backtested NEGATIVE — left OFF there;
# see PIT_codex_fixes/FIXES_AND_TODO.md for the full writeup.)
CHASE_GATE_ENABLED    = True
...
#   logs/YYYY-MM-DD/scalper_v51/{trades,activity}.log + trades.csv
#   logs/scalper_v51_master.csv  + trading.db rows (strategy=scalper_v51)
# ============================================================

_TL = TradeLogger("scalper_v51")
...
# ── Entry funnel diagnostic (ported from V4.3, adapted for V5.1) ──
# Answers "why didn't we trade?" — counts where each scan died in the funnel.
# One row per instrument per day, appended to logs/scalper_v51_funnel.csv.
FUNNEL_COLUMNS = [
    "date", "instrument", "scans", "setup_seen", "taken",
    "adx_nan", "adx_low", "no_setup",
    "confirm_pending", "confirm_inconsistent",
    "reentry_block", "entry_failed",
]


def new_funnel():
    return dict.fromkeys(FUNNEL_COLUMNS[2:], 0)


def write_funnel(name, funnel):
    import csv as _csv
    path = _TL.root / "scalper_v51_funnel.csv"
    row = {"date": get_ist_now().strftime("%Y-%m-%d"), "instrument": name, **funnel}
    new_file = not path.exists()
    try:
        with open(path, "a", newline="", encoding="utf-8") as fh:
            w = _csv.DictWriter(fh, fieldnames=FUNNEL_COLUMNS)
            if new_file:
                w.writeheader()
            w.writerow(row)
        log(name, "Funnel: " + " ".join(f"{k}={funnel[k]}" for k in FUNNEL_COLUMNS[2:]))
    except Exception as e:
        log(name, f"Funnel write failed: {e}")
...
def log_day_start(name):
    ...
    tlog("")
    tlog(sep)
    tlog(f"# {name} V5.1 RIDER Started — {get_ist_now().strftime('%Y-%m-%d')}")
    tlog(f"# Entry: {ENTRY_TIME} | Last entry: {LATE_ENTRY_CUTOFF} | "
         f"Exit: {EXIT_TIME} | Max Trades: {MAX_TRADES} | Re-entry guard: {'ON' if REENTRY_GUARD else 'OFF'}")
    tlog(sep)
    tlog("")
    alog(f"# {name} V5.1 Activity Log — {get_ist_now().strftime('%Y-%m-%d')}")
    alog("")
...
def execute_market_order(symbol, exchange, action, quantity, fallback_price=None):
    ...
    if not order_id:
        return None
    return verify_fill(order_id, fallback_price=fallback_price, fallback_qty=quantity, strategy=STRATEGY_TAG)
...
def log_trade(
    ...
    lines = [
        "",
        sep,
        f"TRADE #{trade_num} | {name} V5.1 | {entry_time.strftime('%Y-%m-%d')} | {entry_time.strftime('%H:%M:%S')}",
        sep,
        "",
...
def run_instrument(name, cfg):
    log(name, f"V5.1 RIDER Started | Qty:{cfg['quantity']} "
              f"InitSL:{cfg['initial_sl_pts']}pts Trail:{cfg['trail_giveback_pts']}pts | "
              f"NO target, NO ADX ceiling | Re-entry guard:{'ON' if REENTRY_GUARD else 'OFF'}")

    total_pnl        = 0
    trade_count      = 0
    last_sl_time     = None
    last_exit_time   = None
    # Re-entry guard state: spot level at which we last ENTERED each direction.
    # A same-direction re-entry must beat this by orb_buffer (fresh extreme).
    last_entry_spot  = {"CE": None, "PE": None}
    ...
    funnel           = new_funnel()   # entry-funnel diagnostic (see write_funnel)
    ...
    while True:
        ...
        if now_str >= EXIT_TIME or squareoff_requested():
            _why = "Square-Off requested" if now_str < EXIT_TIME else "Session ended"
            tradelog(name, f"{_why} | Trades:{trade_count} | P&L: Rs {total_pnl:.0f}")
        ...
        # ── RE-ENTRY GUARD (V5.1) ────────────────────────────
        # If we already traded THIS direction today, only re-enter once spot has
        # pushed to a fresh extreme beyond the previous same-direction entry by
        # orb_buffer points. Blocks the "chase a move that already ran" pattern
        # (29-Jun/01-Jul re-entry losers) while still allowing genuine trend
        # continuation. The opposite direction is never blocked.
        if REENTRY_GUARD and last_entry_spot[option_type] is not None:
            ref = last_entry_spot[option_type]
            gbuf = cfg["orb_buffer"]
            fresh = (latest > ref + gbuf) if option_type == "CE" else (latest < ref - gbuf)
            if not fresh:
                funnel["reentry_block"] += 1
                need = f">{ref + gbuf:.0f}" if option_type == "CE" else f"<{ref - gbuf:.0f}"
                log(name, f"RE-ENTRY BLOCKED {option_type} — spot {latest:.0f} not a fresh "
                          f"extreme (need {need} vs last {option_type} entry {ref:.0f}). Chop guard.")
                signal_history.clear()
                time.sleep(SIGNAL_INTERVAL)
                continue

        # ── CHASE GATE ────────────────────────────────────────
        # Block a 2nd+ same-direction entry if the PRIOR same-direction trade
        # WON. Backtest-validated (+Rs2,460 on V5.1 history) — see config note.
        if CHASE_GATE_ENABLED and last_dir_result[option_type] is True:
            log(name, f"CHASE GATE BLOCKED {option_type} — previous {option_type} "
                      f"trade won; this would be chasing a paid-off move.")
            signal_history.clear()
            time.sleep(SIGNAL_INTERVAL)
            continue
        ...
        signal_history.clear()
        # Record this direction's entry spot for the re-entry guard (V5.1).
        last_entry_spot[option_type] = latest
        ...
        def finalize(exit_reason, pnl_pts, pnl_rs, exit_premium):
            ...
            last_dir_result[option_type] = pnl_rs > 0   # feed the chase gate
        ...
        while position_open:
            ...
            # Hard time exit (session close OR operator square-off)
            _sqoff = squareoff_requested()
            if now_str >= EXIT_TIME or _sqoff:
                _er = "Square-Off" if (_sqoff and now_str < EXIT_TIME) else "Hard Exit"
                ...
                position_open  = False
                tradelog(name, f"{_er.upper()} | Rs {pnl_rs:.0f} | Total: Rs {total_pnl:.0f}")
                finalize(_er, pnl_pts, pnl_rs, curr)
                break
            ...
            # fast-poll while trailing: that's when a reversal gapping through
            # the trail costs the most (see MONITOR_INTERVAL_HOT note).
            time.sleep(MONITOR_INTERVAL_HOT if trail_active else MONITOR_INTERVAL)

        time.sleep(SIGNAL_INTERVAL)

    write_funnel(name, funnel)   # dump the "why didn't we trade" diagnostic
...
def main():
    tlog("=" * 60)
    tlog("Scalper V5.1 — RIDER Edition + Re-Entry Guard — NIFTY + SENSEX")
    tlog("Breakout entry + ride-the-trend exit (no fixed target, no ADX ceiling)")
    tlog(f"Re-entry guard: {'ON' if REENTRY_GUARD else 'OFF'} (fresh-extreme required for same-dir re-entry)")
    tlog("=" * 60)
    tlog(f"Log folder : {_TL._date_folder()}")
    tlog(f"Master CSV : {_TL._master_csv_path()}")
    tlog("")
    alog("=" * 60)
    alog("V5.1 Activity Log Started")
    alog("=" * 60)
    alog("")
    ...
    tlog("=" * 60)
    tlog("V5.1 Rider complete for the day.")
